...
import Payment from './payment'; // Ensure this path is correct

export function Cart({ isOpen, closeCart }) {
  const [cartItems, setCartItems] = useState([]);
  const [totalPrice, setTotalPrice] = useState(0);
  const [isCheckout, setIsCheckout] = useState(false); // State to control payment view
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      {isCheckout && (
        <Payment closePayment={() => setIsCheckout(false)} />
      )}
    </div>
  );
}
